...
# ---------- KPI Counts (Plotly indicator figures) ----------

def _indicator_response(value: int, title: str) -> JSONResponse:
    return JSONResponse(content={
        "figure": {
            "data": [
                {"type": "indicator", "mode": "number", "value": int(value)}
            ],
            "layout": {"title": title}
        }
    })
...
@router.get("/kpis/incident-total")
async def kpi_incident_total():
    df = get_incident_df()
    count = 0 if df is None else int(len(df))
    return _indicator_response(count, "Incidents Total")


@router.get("/kpis/hazard-total")
async def kpi_hazard_total():
    df = get_hazard_df()
    count = 0 if df is None else int(len(df))
    return _indicator_response(count, "Hazards Total")


@router.get("/kpis/audit-total")
async def kpi_audit_total():
    df = get_audit_df()
    count = 0 if df is None else int(len(df))
    return _indicator_response(count, "Audits Total")


@router.get("/kpis/inspection-total")
async def kpi_inspection_total():
    from ..services.excel import get_inspection_df  # local import to avoid circulars at module import time
    df = get_inspection_df()
    count = 0 if df is None else int(len(df))
    return _indicator_response(count, "Inspections Total")


@router.get("/kpis/actions-outgoing-total")
async def kpi_actions_outgoing_total():
    """Return only the total count of outgoing actions as an indicator figure."""
    inc = get_incident_df()
    if inc is None:
        inc = pd.DataFrame()
    haz = get_hazard_df()
    ...
    def _count_from_df(df: pd.DataFrame) -> int:
        if df is None or df.empty:
            return 0
        status_s = _first_existing(df, ["status", "audit_status", "hazard_status", "workflow_status"])
        if status_s is None:
            return 0
        status = _ensure_str_list(status_s)
        keys = ["open", "progress", "identified", "review", "mitigat"]
        return sum(1 for st in status if any(k in (st or "").lower() for k in keys))

    total = _count_from_df(inc) + _count_from_df(haz)
    return _indicator_response(int(total), "Outgoing Actions")
