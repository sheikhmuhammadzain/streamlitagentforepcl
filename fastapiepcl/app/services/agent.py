...
def _normalize_openrouter_model(model: str) -> str:
    ...
    - x-ai/grok-4-fast:free (FREE - recommended for development)
    - x-ai/grok-2-latest
    - x-ai/grok-2-1212
    - x-ai/grok-2-vision-1212

    Deprecated/invalid examples that we remap:
    - x-ai/grok-beta -> x-ai/grok-4-fast:free
    """
    if not model:
    ...
    aliases = {
        ...
        "x-ai/grok": "x-ai/grok-4-fast:free",
        "x-ai/grok-beta": "x-ai/grok-4-fast:free",
        "x-ai/grok-latest": "x-ai/grok-4-fast:free",
    }
...
def _openrouter_fallback_models(primary: str) -> list[str]:
    ...
    OPTIMIZED: If primary is x-ai/grok-4-fast:free, return ONLY that model for speed.
    """
    normalized = _normalize_openrouter_model(primary)
    
    # Fast path: if requesting the working model, use ONLY it
    if normalized == "x-ai/grok-4-fast:free":
        return ["x-ai/grok-4-fast:free"]
    
    # Fallback path: try multiple models
    fallbacks = [
        normalized,
        "x-ai/grok-4-fast:free",  # Primary free Grok
        "x-ai/grok-2-latest",
        "x-ai/grok-2-1212",
...
def _generate_conversational_response(query: str, model: str = "x-ai/grok-4-fast:free") -> str:
...
async def ask_openai_stream(question: str, context: str, *, model: str = "x-ai/grok-4-fast:free", code_mode: bool = False, multi_df: bool = False) -> AsyncGenerator[str, None]:
...
async def generate_agent_response_stream(query: str, *, dataset: str = "incident", model: str = "x-ai/grok-4-fast:free", max_retries: int = 3) -> AsyncGenerator[Dict[str, Any], None]:
    """
